...
export function createApiClient(): ApiClient {
	...
	async function login(email: string, password: string) {
		// return (await httpClient.post<void>("/auth/login")).data;

		// mock
		if (email !== mockMember.email) throw { code: "UNAUTHORIZED", message: "Invalid email or password" };
		return mockMember;
	}
	...
	async function getPlatform(platformId: string) {
		...
		return platformMocks.find((p) => p.id === platformId) || platformMock;
	}
	...
	async function createApiKey(platformId = "", keyType = "public") {
		...
		return mockApiKeys[mockApiKeys.length - 1];
	}
	...
	async function getApiKey(keyId: string) {
		...
		return mockApiKeys.find((k) => k.id === keyId) || mockApiKey;
	}
	...
	async function deleteApiKey(keyId: string) {
		...
		return mockApiKeys.filter((k) => k.id !== keyId);
	}
